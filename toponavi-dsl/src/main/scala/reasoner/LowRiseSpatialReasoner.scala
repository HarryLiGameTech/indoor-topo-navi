...
case class DirectionArrow(
  ...
  def back: TopoNode = {
    if (invertFacing) then reference else anchor
  }

  def front: TopoNode = {
    if (invertFacing) then anchor else reference
  }
}
...
case class CoordEstimationResult(
  coordMap: Map[String, Map[TopoNode, TpccCoord]],
  errors: List[String]
)
